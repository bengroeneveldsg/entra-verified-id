...
1. **Direct login** — user visits a login page, scans QR, and receives a session (e.g. for `example.com`)
...
└──────────────────────────────────────────────────────────────────────
            │                                   │
            ▼ VPC Origin (pl-0redacted01)           ▼ S3 OAC
   ┌─────────────────┐                 ┌─────────────────────┐
...
- **VPC Origin** connects CloudFront to the internal frontend ALB inside the private VPC. This is what allows an ALB with no public IP to serve internet traffic; CloudFront routes requests into the VPC over AWS's private backbone using prefix list `pl-0redacted01`.
...
**Resource:** Distribution ID `EREDACTEDCFDIST`
...
|---------|----------------------|-----------------|
| **CloudFront** | Global CDN + TLS termination | Distribution `EREDACTEDCFDIST` |
...
- ALB security group allows inbound only from CloudFront VPC Origins prefix list (`pl-0redacted01` in ap-southeast-1)
- Fargate tasks run in private subnets with `assignPublicIp: true` — this is because the frontend VPC (`vpc-0redacted00000002`) has an IGW, which is required for the CloudFront VPC Origin feature
...
Deploys the FastAPI admin console to ECS Fargate in the admin VPC (`vpc-0redacted00000001`). An AWS WAF Web ACL with an IP allowlist (VPN CIDR) guards the internal ALB — no request reaches the container without first passing WAF.
...
|-----|----|---------|--------|
| Frontend | `vpc-0redacted00000002` | Public frontend Fargate, ALB | IGW (required for VPC Origin) |
| Admin | `vpc-0redacted00000001` | Admin Fargate, admin ALB | Cloud WAN |
...
```
Internet → CloudFront → [pl-0redacted01 prefix list] → ALB SG → ALB → Fargate SG → Fargate tasks
```
...
**Distribution ID:** `EREDACTEDCFDIST`
...
- AWS account `123456789012` with appropriate IAM permissions
...
CDK_DEFAULT_ACCOUNT=123456789012 CDK_DEFAULT_REGION=ap-southeast-1 \
  npx cdk bootstrap aws://123456789012/ap-southeast-1 --profile your-profile
...
FRONTEND_VPC_ID=vpc-0redacted00000002
FRONTEND_SUBNET_IDS=subnet-xxx,subnet-yyy
ADMIN_VPC_ID=vpc-0redacted00000001
...
CF_PREFIX_LIST_ID=pl-0redacted01
```
...
| `HOSTING_BUCKET` | CDK | `entra-vid-hosting-123456789012-v2` |
| `STAGE` | CDK | `v2` |
| `LOG_LEVEL` | CDK | `INFO` |
